...
if (isset($_POST['simpan']))
{
	...
	$subbelanja = $_POST['belanja_kepda'];
	...
	$query = "insert into tm_kepda(belanja_kepda,tgl_program,nama_kegiatan,total_anggaran,realisasi,sisa_anggaran,persen) 
	values('$subbelanja','$tanggal','$nama_kegiatan','$total_anggaran','$realisasi','$total_sisa_anggran','$total_persen')";
	$result = mysqli_query($link, $query);
	if ($result)
	{
		echo "<script>alert('sukses');window.location.assign(\"page.php?page=dataKepda\")</script>";
	}
	else {
		echo "<script>alert('gagal');window.location.assign(\"page.php?page=addKepda\")</script>";
	}
}
		...
		<form method="post">
			<div class="form-group">
			  ...
			  <select name="belanja_kepda" class="form-control" style="width: 70%">
			  	...
			  	<option value="2">Belanja Operasional</option>
